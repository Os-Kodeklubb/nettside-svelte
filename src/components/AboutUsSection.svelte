<section class="explanation" id="om-oss">
  <div class="row">
    <div class="box item-list">
      <h2 class="item" style="font-size: 70px; color: var(--secondary-color)">
        Hva er Os Kodekubb?
      </h2>
      <div class="divider" />
      <p class="item" style="margin-top:auto; margin-bottom:auto;">
        Os Kodeklubb er en klubb for de over 18 år med interesse for koding, og
        som søker et sosialt kodefellesskap. Klubben er åpen både for
        nybegynnere og erfarne kodere. På kodeonsdagene fokuserer vi på å lære
        koding eller lære mer koding.
      </p>
    </div>
  </div>
</section>

<style lang="css">
  .box {
    padding: 70px;
  }

  .item-list p {
    color: #3c4856c2;
    font-size: 1.5rem;
    line-height: 1.5;
    text-align: left;
    margin: 0.5rem 0 2.3625rem 0;
  }

  .item-list {
    display: flex;
    flex-wrap: wrap;
    justify-content: space-between;
    margin: 0;
    align-items: center;
  }

  .item {
    width: 500px;
  }

  @media only screen and (max-width: 767px) {
    .item {
      width: 500px;
    }

    .divider {
      border-left: 1px solid #000;
      height: 400px;
      display: none;
    }
  }

  @media only screen and (min-width: 768px) {
    .item {
      width: 250px;
    }

    .divider {
      border-left: 1px solid #000;
      height: 600px;
    }
  }

  @media only screen and (min-width: 924px) {
    .item {
      width: 300px;
    }

    .divider {
      border-left: 1px solid #000;
      height: 500px;
    }
  }

  @media only screen and (min-width: 1087px) {
    .item {
      width: 350px;
    }
    .divider {
      border-left: 1px solid #000;
      height: 400px;
    }
  }

  @media only screen and (min-width: 1200px) {
    .item {
      width: 400px;
    }
    .divider {
      border-left: 1px solid #000;
      height: 400px;
    }
  }

  @media only screen and (min-width: 1400px) {
    .item {
      width: 450px;
    }
    .divider {
      border-left: 1px solid #000;
      height: 300px;
    }
  }
  @media only screen and (min-width: 1401px) {
    .item {
      width: 500px;
    }
    .divider {
      border-left: 1px solid #000;
      height: 300px;
    }
  }

  @media only screen and (min-width: 1740px) {
    .item {
      width: 600px;
    }
    .divider {
      border-left: 1px solid #000;
      height: 300px;
    }
  }
</style>
